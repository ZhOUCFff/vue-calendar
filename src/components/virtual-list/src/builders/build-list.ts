...
const createList = ({
  ...
  return defineComponent({
    ...
    emits: [ITEM_RENDER_EVT, SCROLL_EVT, REACH_BOTTOM_EVT, REACH_TOP_EVT],
    setup(props, { emit, expose }) {
      ...
      const onScroll = (e: Event) => {
        ...
        const scrollHeight = unref(estimatedTotalSize)
        const horizontal = unref(_isHorizontal)

        const length = horizontal ? width : height
        const scrollOffset = horizontal ? scrollLeft : scrollTop

        if (scrollOffset + length === scrollHeight) {
          emit(REACH_BOTTOM_EVT)
          console.log('到达底部')
        }
        if (scrollOffset === 0) {
          emit(REACH_TOP_EVT)
      ...
      const api = {
        isHorizontal: _isHorizontal.value,
        clientSize,
        estimatedTotalSize,
        windowStyle,
        windowRef,
    ...
    render(ctx: any) {
      ...
      const listContainer = h(
        Container as VNode,
        {
          ...
          style: windowStyle,
          onScroll,
          // onWheel: e => (isHorizontal ? onWheel(e, listContainer) : null),
          ref: 'windowRef',
          key: 0
        },
